...
const server = Bun.serve({
  ...
  fetch: async (request) => {
    ...
    const logRequest = (status: number, note: string, targetUrl?: URL) => {
      const durationMs = Date.now() - startedAt;
      const targetInfo = targetUrl ? ` target=${targetUrl.href}` : "";
      console.log(
        `${request.method} ${request.url} -> ${status} ${note}${targetInfo} ${durationMs}ms`,
      );
    };

    if (request.method === "CONNECT") {
      logRequest(405, "connect_not_supported");
    ...
    if (!targetUrl) {
      logRequest(400, error ?? "bad_request");
    ...
    if (!allowed.allowed) {
      logRequest(403, allowed.reason ?? "forbidden", targetUrl);
